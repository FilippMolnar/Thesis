...
\section*{TR-MISR versus MISRGRU}
...
\section*{Streaming MISRGRU and real-time deployment}
...
At 0.2\,px/frame motion speed the DPA variant reached an RSP of 0.889 on fast-high-SNR
conditions, close to the static baseline of 0.905, while the finetune-only variant scored
0.863.
Visual inspection confirmed that the DPA model produces less ghosting and blur effects
from misaligned hidden states.

\todo{change retain competitive RSP}
Both motion-capable models retain competitive RSP on static test data; the degradation is
concentrated in slow-low-SNR conditions where temporal averaging is most valuable.
For deployment, the unmodified streaming model is preferred when the sample is stationary
or drift is sub-pixel; the DPA variant should be selected when the specimen exhibits
...
delivering continuous SR video at 11\,ms per frame, comfortably within the 25\,ms budget
of a 40\,fps acquisition, while converging to the quality of the foundational batch model.
