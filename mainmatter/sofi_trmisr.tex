...
This chapter describes the our contributions to this thesis.
...
Here we describe only what is new: the modifications made when adapting those architectures to
...
\section{Modifications to TR-MISR}
\label{sec:adaptation}

\paragraph{Encoder.}
Both models share the same encoder design, adapted from the original works by 
\cite{resurf2024,TR-MISR-original}.
The main difference to original TR-MISR paper is that the encoder accepts 
a single fluorescence channel instead of two (NIR and RED), 
and we do not concatenate a median reference image as 
in fluorescence microscopy we not does not benefit from 
co-registration in the same way as satellite imageing.
Channel size is reduced from 64 to 24 to decrease latency and to match the RESURF adaptation~\cite{resurf2024}.

\paragraph{Decoder.}
The original TR-MISR decoder uses a $3\times$ PixelShuffle for the 
$3\times$ upscaling.
...
vector.
Fer every pixel $(h,w)$, in the sequence of $N$ frames:
...
Architecture where many inputs can many produce many outputs would address both 
...
This way the model
could accumulate more context which can help to better reconstruct, while keeping the 
latecy lower per reconstructed image
...
also saving comupute by remembering $h_{k-n} \dots h_{k-1}$.
...
The GAP in RESFURF\cite{resurf2024} is computed as sum of all $h_{1} \dots h_{k}$. 
...
The only architectural change is in how the ConvGRU hidden state is managed.
In the batch model, the hidden state is zeroed at the start of each new sample.
In the streaming model, $\mathbf{h}_t$ is carried across successive frames and never 
reset between time steps, so the GRU accumulates context from the entire history of 
frames seen.
% \begin{equation}
...
Moreover this helps the model to stabilize 
...
A second CNN predicts per-pixel sampling offsets and modulation masks from the
...
entirely and the GRU is initialised from a zero hidden state.
...
In live-cell imaging, organels can move at
...
To make the streaming model robust to this, we introduce \emph{motion augmentation} during
training.
For each training sequence, with probability $p = 0.7$ a smooth drift trajectory is sampled
for both the $x$ and $y$ axes independently.
Each axis trajectory follows a sinusoidal pattern:
\begin{equation}
  d_\text{axis}(t) = A\,\sin(2\pi f t + \varphi),
  \label{eq:trajectory}
\end{equation}
where the frequency $f$ is sampled uniformly in $[1.5/T,\, 3.5/T]$, the phase $\varphi$
uniformly in $[0, 2\pi)$, and the amplitude $A$ is set so the instantaneous speed
$\dot{d}(t)$ stays in the range $[v_\text{min},\, v_\text{max}] = [0.2,\, 1.5]$ pixels/frame.
This covers the realistic biological range from slow axonal transport ($\sim$0.1 px/frame)
to fast organelle movement ($\sim$1.5 px/frame).
Sinusoids are used because they produce smooth, bounded drift that stays within the image field
without accumulating bias in one direction.

Each LR frame $\mathbf{l}_t$ is shifted by the displacement $d(t)$ using bilinear interpolation,
and the static HR ground truth is shifted by the same displacement scaled by the upsampling
factor $\gamma \approx 1.984$.
This produces per-frame HR targets $\hat{\mathbf{Y}}_t$ that match the shifted field of view at
each time step, so the model learns to localise the correct position rather than always predicting
the same static reconstruction.
The remaining 30\% of training batches keep the original static data, which prevents the model
from forgetting to reconstruct well on non-moving sequences.
...
The mathematical definitions of the loss functions are given in Section~\ref{subsec:loss_functions}.
Here we describe the specific configurations used in training and the rationale for choosing them.

We train all models with three loss variants:
\begin{itemize}
  \item \textbf{Fourier}: frequency-domain loss $\mathcal{L}_{\mathcal{F}}$ only, penalising errors
        in both the amplitude and phase of the 2D DFT (Eq.~\eqref{eq:fourier_loss}).
  \item \textbf{L1+Fourier}: the equal-weight combination 
        $\mathcal{L} = \mathcal{L}_{L1} + \mathcal{L}_{\mathcal{F}}$
        (Eq.~\eqref{eq:composite_loss} with $\lambda_1 = \lambda_2 = 1$).
\end{itemize}

The motivation for including a Fourier-domain term is grounded in SOFI physics.
Second-order SOFI recovers spatial frequencies up to $2f_c$ — above the diffraction-limited
cut-off $f_c$ of the raw widefield image.
An $\ell_1$ loss in the spatial domain is dominated by the dense dark background
(which contributes the most pixels), leaving the gradient signal from the super-resolved
high-frequency band relatively weak.
The Fourier loss weights every frequency component equally, directly penalising errors at
the high-frequency band that SOFI is designed to reveal.
This provides a principled reason why the Fourier loss consistently outperforms the spatial
$\ell_1$ baseline on this task, as reported in~\cite{resurf2024, jelle2024}.
Ablation results for all three configurations are given in Appendix~\ref{app:trnet_modifications}.

For the streaming MISRGRU, the same L1+Fourier loss is applied per-frame to each streaming
output $\hat{\mathbf{Y}}_t$, summed over the \texttt{loss\_last\_n} supervision frames at the
end of each TBPTT chunk (Section~\ref{sec:streaming_arch}).
...
early in training.
For the streaming MISRGRU, a single rate of $\eta = 10^{-4}$ is used throughout;
higher rates ($10^{-3}$, $3 \times 10^{-4}$) cause gradient instability during
truncated backpropagation and lead to training collapse.

\paragraph{Learning rate schedule.}
We use cosine annealing with warm restarts: the learning rate is decayed from its
initial value to zero following a cosine curve over the full training run, then reset
and decayed again.
This was found to produce more stable convergence than ReduceLROnPlateau on the plateau
of the validation loss curve.
...
64$\times$64 LR patches with 8 frames per sample.

\paragraph{Training duration.}
Models are trained for up to 200 epochs.
For the streaming MISRGRU, training sequences are split into chunks of 16 frames
(\texttt{chunk\_size}) with gradients detached at chunk boundaries (TBPTT).
Only the last 20 frames of each chunk contribute to the loss (\texttt{loss\_last\_n}),
giving the GRU time to accumulate context before supervision is applied.

\paragraph{Data split.}
Training uses up to 4000 simulated samples from the synthetic dataset described in
Chapter~\ref{ch:datasets}.
A 90/10 random split assigns samples to training and validation sets; the test set is
held out entirely and evaluated only after all hyperparameter choices are finalised.

\paragraph{Hardware.}
TRNet experiments were run on a single NVIDIA RTX A5000 (24\,GB).
Streaming MISRGRU experiments were run on DelftBlue HPC cluster nodes equipped with
NVIDIA A100 GPUs (40\,GB), using Apptainer containers for reproducibility.
