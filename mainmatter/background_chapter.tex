...
This chapter provides the technical foundation needed to follow the rest of the thesis.
It moves from the physics of the imaging problem to the algorithms that this work seeks to
accelerate. We begin with fluorescence microscopy and the diffraction limit that motivates
super-resolution, then introduce super-resolution techniques and difficulties when implementing 
them for live-cell imaging.
We then cover the deep-learning building blocks used later, and close by describing the two
reference architectures this thesis builds on and compares, MISRGRU and TRMISR.
...
(Figure~\ref{fig:rayleigh}).
This gives the minimum resolvable distance:
\begin{equation}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\linewidth]{figures/rayc.png}
  \caption{The Rayleigh criterion. Two point sources are just resolved when the principal
...
The observed image is a combination of shifted, scaled PSFs from every emitter:
\begin{equation}
...
molecule localisation, offering $\sqrt{n}$-fold resolution improvement with $n^{th}$ 
...
SOFI can typically achieve temporal resolution on the order of seconds. 
...
order of seconds per frame, making SOFI not suitable for real-time,
long-duration live-cell imaging. \cite{resurf2024}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/sofi-smlm.png}
  \caption{Comparison of single-molecule localisation (A) and SOFI (B) on a stack of
    diffraction-limited frames. Localisation methods pinpoint the centre of each isolated emitter
    across frames and accumulate the positions into a super-resolved image, requiring sparse,
    well-separated blinking. SOFI instead exploits the temporal correlations of the intensity
    time-traces (C): because signals from distinct emitters are statistically independent, their
    cross-correlation averages to zero, while higher-order cumulants of co-located pixels narrow
    the effective PSF. This recovers the underlying structure even under dense, overlapping
    emission where localisation fails.}
  \label{fig:sofi_smlm}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/sofi-crosscumulant.png}
  \caption{SOFI algorithm: cross-cumulant calculation. The $n^{\text{th}}$-order cross-cumulant
    $\kappa_n$ of a pixel $\delta$ is computed as a weighted sum over all partitions of a set $G$
    of $n$ pixels. The position of pixel $\delta$ is taken as the geometric mean of the $n$ pixels
    within $G$. Choosing different sets of $n$ pixels allows the $n^{\text{th}}$-order cross-cumulant
    to be evaluated on an arbitrarily large pixel grid. Formulas are given for general $n$, with
    sketches for $n = 4$.}
  \label{fig:sofi_crosscumulant}
\end{figure}
...
\hline
STED~\cite{sted1994}       & $<\!50\,\mathrm{nm}$      & Single          & Depletion laser + patterned illum.   \\[2pt]
SIM~\cite{sim2000}         & $\approx120\,\mathrm{nm}$ & 9--15           & Patterned illum.  \\[2pt]
SMLM~\cite{rust_2006_subdiffractionlimit,betzig_2006_imaging} & 10--20\,nm   & $10^5$--$10^6$  & Widefield         \\[2pt]
SOFI~\cite{sofi2009}       & $\approx\!{n}$-fold with postprocessing  & 100--2000        & Widefield         \\[2pt]
\hline
\end{tabular}
\end{table}
...
due to the nature of SOFI reconstruction algorithm. Specifically, second-order cross-cumulants are computed by
pairing pixels within a fixed neighbourhood, with each pair contributing a virtual pixel at its centroid to form a
grid twice as dense as the detector array; the limited pairing range means the outermost pixels cannot form valid
pairs, which trims the doubled grid and yields the $-7$ offset. Thus our models need to be designed to produce an output
...
Pearson correlation (RSP) for structural fidelity, commonly used as quality metrics in super-resolution 
microscopy.
...
\subsubsection{SFSRM: Single-frame deep-learning super-resolution microscopy for intracellular dynamics imaging}
Chen et al.~\cite{chen2023sfsrm} proposed single-frame super-resolution microscopy (SFSRM),
which takes the opposite approach to fluctuation-based methods: rather than fusing many frames,
it reconstructs a super-resolved image from a \emph{single} diffraction-limited frame.
The model builds on an enhanced super-resolution generative adversarial network (ESRGAN),
guided by two additions tailored to microscopy data.
First, a subpixel edge map is extracted from the input frame and fed to the network as prior
information.
Second, a multi-component loss combining a multi-scale structural-similarity term with perceptual,
adversarial, and frequency-domain losses is used to recover high-frequency detail while suppressing
the artefacts that adversarial training can introduce.
With these elements SFSRM achieves spatiotemporal resolutions of 30\,nm at 10\,ms,
enabling prolonged live-cell imaging of dynamics such as vesicle transport along microtubules
and endosome fusion and fission.
However, the method has a prerequisite input signal-to-noise ratio (SNR) of about~15, so under the
low-SNR conditions typical of high-speed live-cell imaging it requires a separate signal-enhancement
network (a second ESRGAN, denoted SEN) to denoise the frame before reconstruction.
This added denoising stage, together with the large size of the adversarial generator, has been
reported to result in a computational latency on the order of hundreds of milliseconds in inference
time, preventing real-time application~\cite{resurf2024}.
...
The overall structure of the Transformer encoder, with its stacked self-attention and
feed-forward blocks, is shown in Figure~\ref{fig:transformer_architecture}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.55\linewidth]{figures/transformer_architecture.png}
  \caption{The Transformer architecture~\cite{vaswani2017attention}. Each encoder layer
    combines multi-head self-attention with a position-wise feed-forward network, using
...
(Figure~\ref{fig:mha_architecture}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.45\linewidth]{figures/MHA_architecture.png}
  \caption{Multi-head attention~\cite{vaswani2017attention}. The queries, keys, and values are
    linearly projected into subspaces, scaled dot-product attention is computed in each head
    in parallel, and the head outputs are concatenated and projected back to the input dimension.}
  \label{fig:mha_architecture}
\end{figure}
...
The choice of a multi-frame architecture follows directly from the nature of the SOFI task.
The point spread function is identical at every time step, so a
single frame carries no super-resolution information. It is simply a diffraction-limited image.
The resolution gain that SOFI exploits lives entirely in the temporal statistics of fluorophore
blinking. 
A network that processes frames jointly can learn to recover the same statistical information
that the classical SOFI algorithm computes explicitly. 
This is why both reference architectures fuse a sequence of $N$ frames, and it reframes the
central design question as how those frames should be fused, the contrast between sequential
recurrent fusion and parallel attention-based fusion explored in this thesis.
...
fluctuations and and is invariant to the ordering of frames. 
...
\textbf{Decorrelation analysis~\cite{descloux2019decorrelation}} is commonly used in
