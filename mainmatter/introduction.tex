...
the studies sample. Live-cell imaging is therefore constrained to low
excitation power and short exposure times, which limit the number of photons
collected per frame. This
low photon budget translates directly into a capturing images with 
...
\begin{enumerate}
  \item Adaptation of the TR-MISR transformer fusion module to second-order SOFI
    super-resolution, including a single-channel encoder, a decoder for the non-integer
    $2n{-}7$ upscaling, a corrected attention-score scaling, and a CrossAttentionAggregator
    that separates frame-to-frame communication from final feature aggregation
    (Chapter~\ref{ch:method}).
  \item A systematic comparison of TR-MISR and SOFI-MISRGRU across various blinking and 
  SNR conditions,
    showing that attention-based fusion holds its quality under low-SNR, slow-blinking
    acquisitions where the GRU degrades (Chapter~\ref{ch:results}).
  ...
  \item A streaming SOFI-MISRGRU that produces one super-resolved output per input frame,
    reaching the batch baseline and improving with more accumulated frames. 
